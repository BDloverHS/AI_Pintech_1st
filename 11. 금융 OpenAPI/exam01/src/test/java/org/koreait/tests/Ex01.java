...
public class Ex01 {
    ...
    void test1() throws Exception {
        ...
        String json = om.writeValueAsString(form); // 자바 객체가 JSON 문자열로 변환
        System.out.println(json);

        // JSON 문자열 -> 자바 객체 변환
        RequestJoin form2 = om.readValue(json, RequestJoin.class); // 단일 클래스 형태의 자료ㅕ형으로 변환 Class 클래스가 매개변수
    ...
    void test3() throws Exception {
        // UriComponents url = UriComponentsBuilder.fromUri(URI.create("https://naver.com")).build();
        UriComponents url = UriComponentsBuilder.fromUriString("https://naver.com")
                .queryParam("k1", "v1")
                .queryParam("k2", "v2")
                .queryParam("k3", "한글")
                .fragment("header")
                .encode() // 멀티바이트 문자 -> 16진수 형태로 변경 - URL 인코딩
                .build();
